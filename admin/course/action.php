...
if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['action'])) {
	...
	if ($_POST['action'] == 'delete') {
		if ($ids) {
			foreach ($ids as $id) {
				$query = "DELETE FROM satt_courses WHERE id = '$id'";
		...
		if ($ids) {
			foreach ($ids as $id) {
				$query = "UPDATE satt_courses SET course_status = 1 WHERE id = '$id'";
		...
		if ($ids) {
			foreach ($ids as $id) {
				$query = "UPDATE satt_courses SET course_status = 0 WHERE id = '$id'";
				$result = $db->update($query);
			}
		...
		if ($ids) {
			foreach ($ids as $id) {
				$query = "SELECT course_status FROM satt_courses  WHERE id = '$id'";
				...
				$query = "UPDATE satt_courses SET course_status = $status WHERE id = '$id'";
				$result = $db->update($query);
			}
			die(json_encode(['message' => '(' . count($ids) . ') ' . (count($ids) == 1 ? 'Course' : 'Courses') . ' Status Toggled Successfull']));
		}
		http_response_code(500);
		die(json_encode(['message' => 'Something Happend Wrong. Please Try Again Later']));
