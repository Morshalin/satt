...
Session::checkSession('admin', ADMIN_URL . '/existing-software-report', 'existing-software-report');
		...
		if (isset($_POST['submit'])) {
			...
			if ($report_type == 'payment') {
				...
				$query = "SELECT * FROM new_product_order";
				...
				if ($get_product) {
					...
					while ($row = $get_product->fetch_assoc()) {
						$id = $row['id'];
						$paid = 0;
						$query = "SELECT * FROM new_product_pay WHERE new_product_order_id = '$id'";
						...
						$yearly_pay_amt = 0 ;
						$query = "SELECT * FROM new_product_yearly_charge_pay WHERE new_product_order_id = '$id'";
						...
						$query = "SELECT * FROM new_product_installation_pay WHERE new_product_order_id = '$id'";
						...
						if ($get_installation_pay_info) {
							while ($installation_pay = $get_installation_pay_info->fetch_assoc()) {
								$pay_date = date("Y-m-d", strtotime($installation_pay['date']));
								$pay_date = strtotime($pay_date);
								if ($pay_date >= $from_date && $pay_date <= $to_date) {
						...
						if ($paid > 0 ||  $yearly_pay_amt > 0 || $installation_pay_amt > 0) {
							...
							$data .='  <tr style="color:black" align="left">
											...
											<td>'.$row['expected_name_software'].'</td>
											<td>'.ucwords(implode(' ',explode('_',$row['selling_method']))).'</td>
											<td>'.$paid.'</td>
