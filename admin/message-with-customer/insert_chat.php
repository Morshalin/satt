...
if (isset($_POST['to_user_id'])) {
	...
	if ($get_customer_info) {
		$get_customer_info = $get_customer_info->fetch_assoc();
		$customer_name = $get_customer_info['name'];
	}

if ($chat_message !='') {
	$query = "INSERT INTO admin_customer_chat 
			  (from_user_id,from_whom,to_user_id,to_whom,chat_message,seen_status_admin,seen_status_customer,date)
			  VALUES 
			  ('$admin_id',	'$from_whom',	'$to_user_id',	'$to_whom',	'$chat_message','1','0',now())";
}else{
	die();
}
...
if (isset($_POST['to_user_id_seen_status'])) {
	...
	$query = "UPDATE admin_customer_chat SET seen_status_admin = '1' WHERE (from_user_id = '$admin_id' AND to_user_id = '$to_user_id') OR (from_user_id = '$to_user_id' AND to_user_id = '$admin_id')";
