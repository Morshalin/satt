...
if ($result) {
	while ($row = mysqli_fetch_assoc($result)) {
      ...
      "create_date" => '<strong>' . $row['create_date'] . '</strong>',
        ...
        <input type="checkbox" class="form-check-status-switchery" id="change_status" data-id="' . $row['id'] . '" data-status="' . $row['status'] . '" data-url="' . ADMIN_URL . '/customertype/ajax.php?customertype_id=' . $row['id'] . '&action=status&status=' . $row['status'] . '"' . ($row['status'] == 1 ? 'checked' : '') . ' data-fouc >
        </label>
        	',
		);
		$i++;
