...
ajax();
Session::checkSession('admin', ADMIN_URL . '/cancel-new-order');
## Read value
$draw = $_GET['draw'];
$row = $_GET['start'];
...
## Fetch records
=================================================================================*/
$query = "select * from new_product_order WHERE  confirmation_status = '0' AND delivery_status = '0' AND cancel_status = '1'" . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
		...
		if ($row['agent_id']) {
	      ...
	      $agent_phn = 'N/A';
		    ...
		    $data[] = array(
			  ...
			  "action" => '
              <span class="dropdown-item bg-success" id="content_managment" data-url="' . ADMIN_URL . '/cancel-new-order/show.php?new_order_id=' . $row['id'] . '"><i class="icon-eye"></i> View</span>
        ',
		);
		$i++;
	}
