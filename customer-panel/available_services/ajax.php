...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$customer_query = "SELECT * satt_customer_informations WHERE id ='$customer_id'";
	$customer_result = $db->select($customer_query);
	if ($customer_result) {
		$customer_data = $customer_result->fetch_assoc();
	}

	$agent_query = "SELECT * agent_client WHERE client_id ='$customer_id'";
	$agent_result = $db->select($agent_query);
	if ($agent_result) {
		$agent_data = $agent_result->fetch_assoc();
		$agents_id = $agent_data['agent_id'];
	}else{
		$agents_id='';
	}
	...
	$custimer_number = $customer_data['number'];
	$agent_id = $agents_id;
	$prodict_id = $software_details_id;
	...
	$pay_amount = $a[1];
	$pay_type = $a[0];
	$installation_charge = $_POST['installation_charge'];
