...
ajax();
Session::checkSession('agent-panel', AGENT_URL . '/pending_product');
## Read value
$draw = $_GET['draw'];
$row = $_GET['start'];
...
$query = "select * from satt_order_products WHERE  roll = 0 and status = 0 and agent_id='$agent_id' " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
...
if ($result) {
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $data[] = array(
      ...
      "status" => '<strong class="bg-success p-1">Pending</strong>',
        ...
        <div class="list-icons" id="action_menu_' . $row['id'] . '">
          <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <span class="dropdown-item" id="content_managment" data-url="' . AGENT_URL . '/pending_product/show.php?order_id=' . $row['id'] . '"><i class="icon-eye"></i> View</span>


             <span class="dropdown-item" id="content_managment" data-url="' . AGENT_URL . '/pending_product/reasion.php?order_id=' . $row['id'] . '"><i class="icon-cross"></i> Cancel Order</span>
            </div>
          </div>
        </div>
        ',
