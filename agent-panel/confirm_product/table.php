...
ajax();
Session::checkSession('agent-panel', AGENT_URL . '/confirm_product');
## Read value
$draw = $_GET['draw'];
$row = $_GET['start'];
...
=================================================================================*/
$agent_id = $user['id'];
$query = "select * from satt_order_products WHERE roll = 0 and status = 1 and agent_id ='$agent_id' " . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
...
if ($result){
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    $data[] = array(
      ...
      "agent_comission" => '<strong>' . $row['agent_comission'] . '</strong>',
        ...
        <div class="list-icons" id="action_menu_' . $row['id'] . '">
          <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <span class="dropdown-item" id="content_managment" data-url="' . AGENT_URL . '/confirm_product/show.php?order_id=' . $row['id'] . '"><i class="icon-eye"></i> View</span>

              <span class="dropdown-item" id="content_managment" data-url="' . AGENT_URL . '/confirm_product/user_manual.php?product_id=' . $row['product_id'] . '"><i class="icon-tv"></i> user Manual </span>
