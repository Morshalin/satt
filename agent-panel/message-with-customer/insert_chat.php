	...
	if ($insert_chat) {
		...
		if ($get_chat_history) {
			...
			while ($row = $get_chat_history->fetch_assoc()) {
				$user_name = '';
				if ($row['from_user_id'] == $agent_id && $row['from_whom']=='agent') {
					$user_name = '<b class="text-success">you</b>';
				}else{
					$user_name = '<b class="text-danger">'.$client_name.'</b>';
				}
				$output .= '
...
$query = "SELECT * FROM agent_customer_chat WHERE (from_user_id = '$agent_id_get_info' AND to_user_id = '$to_user_id_get_info') OR (from_user_id = '$to_user_id_get_info' AND to_user_id = '$agent_id_get_info')  ORDER BY id DESC";
		...
		$output = '<ul class="list-unstyled">';
				while ($row = $get_chat_history->fetch_assoc()) {
					$user_name = '';
					if ($row['from_user_id'] == $agent_id_get_info && $row['from_whom']=='agent') {
						$user_name = '<b class="text-success">you</b>';
					}else{
						$user_name = '<b class="text-danger">'.$client_name.'</b>';
					}
...
if (isset($_POST['to_user_id_seen_status'])) {
	...
	$query = "UPDATE agent_customer_chat SET seen_status_agent = '1' WHERE (from_user_id = '$agent_id' AND to_user_id = '$to_user_id') OR (from_user_id = '$to_user_id' AND to_user_id = '$agent_id')";
